...
def build_knn_model(test, modelfile):
    knntestdata=[]
    for each in test:
        knntestdata.append(each[2:])
    testdata_knn=np.array(knntestdata).astype(int)

    trainlabels=[]
    modeldata=[]
    mfile=open(modelfile,'r').readlines()
    for line in mfile:
        trainlabels.append(line.split()[0:2])
        modeldata.append(line.split()[2:])
...
def predictlabels(sorted_labels,testdata, k, model):
    ...
    if model == "nearest":
        outputfile='output_nearest.txt'
    elif model=="best":
        outputfile='output_best.txt'
...
if argtype.lower()=="train":    
    ...
    if model.lower() == "nearest" or model.lower() == "best":
        traindata=train_knn(filetype, modelfile)
    
    elif model.lower() == "adaboost":
        colnames=[i for i in range(-2,192)]
        train_file=pd.read_csv(filetype, header=None, delimiter=' ', names=colnames)
    ...
    if model.lower() == "nearest" or model == "best":
        k=48
        testdata=load_testdata(filetype)
        (knn_model, labels)=build_knn_model(testdata, modelfile)
        sorted_labels = sortlabels(knn_model, labels)
        accuracy=predictlabels(sorted_labels,testdata,k, model)
        print("The accuracy for the KNN Algorithm is",round(accuracy,2),"% at K =",k)

    elif model.lower() == "adaboost":
        colnames=[i for i in range(-2,192)]
        test_file=pd.read_csv(filetype, header=None, delimiter=' ', names=colnames)
        ...
        count=0
        output_file = open('output_adaboost.txt',"w+")
        for i,row in test.iterrows():
            output_file.write(test_file.loc[i,-2]+" "+str(result[i])+"\n")
            if result[i]==test.loc[i,'labels']:
                count+=1
        output_file.close()
        print("The accuracy for the Adaboost Algorithm is:",round(((count/len(result))*100),2),"%")
