...
1. Training the dataset bc.train:
    ...
    1. initial_probability - This dictionary holds the probability of a POS tag occurring in the beginning of the sentence, P(S_1)
    2. prior_probability - This dictionary holds the probability of a POS tag occurring in the entire training dataset, P(S_i)
    3. emission_probability - This dictionary holds the probability of the word given the POS tag, P(W_i|S_i)
    4. trans_probability - This dictionary holds the probability of the next POS tag given its previous POS tag, P(S_i+1|S_i)
    5. trans3_probability - This dictionary holds the probability of P(S_i+2|S_i+1, S_i) i.e, for eg: P(S_3|S_2, S_1)
...
5. Complex MCMC model (Gibbs Sampling):
    ...
    -Initialize a sample list with random POS tags eg: all 'nouns' for the entire sentence length.
    -Using the initial sample we resample each of the POS tags at each positions starting from left by sampling one POS tag at a time and fixing the rest of the tags.
    -Each POS tags at each positions is resampled for all the POS tags by calculating the transition, emission and initial probabilities.
    -We then calculate the probability of the word for each POS tag and then compare each probability with the random probability to select which POS to be selected for the current word.
    -The steps 2, 3 and 4 are repeated for 500 iterations with burnin iterations of 100 and append each sampled list to another sample list.
    -We then pick the most frequently occurred POS tag for each word and append it to the final "mcmcpos" list.
    -We are using 3 state transition probabilities, P(S3|S2,S1) to calculate the probabilities
...
==> So far scored 2000 sentences with 29442 words.
   ...
   0. Ground truth:      100.00%              100.00%
         1. Simple:       93.92%               47.45%
            2. HMM:       95.06%               54.45%
        3. Complex:       95.13%               53.25%
----

8. Conclusion:
    We conclude that the HMM Viterbi performs better followed by MCMC and Simplified because Viterbi implements backtracking to find the most likely sequence,
    whereas MCMC genefrates random samples and conergence point cannot be determined beforehand.
********************************************************************REPORT********************************************************************
'''
...
class Solver:
    ...
    # Initializing a small probability if in case a state/POS does not exist in the training dataset.
    def __init__(self):
       ...
       # Calculating for t=0
        ...
        for t in range(1, len(sentence)):
            ...
            for currst in prior_probability.keys():
                #Calculating the product of viterbi value at t-1 and transition probability for all 12 POS tags and pick the max value
                p=[]
    ...
    def generate_sample(self, sentence, sample):
        ...
        for i in range(sentence_len):
            ...
            probabilities = []
            poslist=[]
            for pos in prior_probability.keys(): # Iterating for each POS tag
                possample[i]=pos
                currpos=possample[i]
                word=sentence[i]
                ...
                #Calculating the probabilities at index =0
                #Also calculating for the special cases when the sentence length is 1, 2 and greater than 2
                if i==0:
                    ip = initial_probability[currpos]
                    if sentence_len==1:
                        p=ip*ep
                ...
                #Calculating the probabilities at index=1
                #Also calculating for the special cases when the sentence length is 2, 3 and greater than 3
                elif i==1:
                    ...
                    if sentence_len==2:
                        ...
                        tp1 = trans3_probability[prevpos][currpos][nextpos] if prevpos in trans3_probability.keys() and currpos in trans3_probability[prevpos].keys() and nextpos in trans3_probability[prevpos][currpos].keys() else self.smooth_prob
                        #P(S3|S2, S1)
                        tp2 = trans3_probability[currpos][nextpos][nextnextpos] if currpos in trans3_probability.keys() and nextpos in trans3_probability[currpos].keys() and nextnextpos in trans3_probability[currpos][nextpos].keys() else self.smooth_prob
                        p = ep*tp1*tp2

                #Calculating the probabilities at index greater than 1 and 2 less than sentence length
                elif i>1 and i<sentence_len-2:
                ...
                #Calculating the probabilities at index=sentence_len -2, i.e the penultimate state
                elif i==sentence_len-2:
                    prevprevpos=possample[i-2]
                    prevpos=possample[i-1]
                    nextpos=possample[i+1]
                ...
                #Calculating the probabilities at index=sentence_len-1 , i.e the last state
                elif i==sentence_len-1:
                    prevprevpos=possample[i-2]
                    prevpos=possample[i-1]
                    tp = trans_probability[currpos][nextpos] if currpos in trans_probability.keys() and nextpos in trans_probability[currpos].keys() else self.smooth_prob
                ...
                #Appending the conditional probabilities and POS tags to a list
                probabilities.append(p)
                poslist.append(pos)

            #Calculating the probabilities and comparing the cumulative probability with a random probability 
            #generated from random.random() to pick the POS to be selected for the current word.
